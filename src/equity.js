...
(function (global) {
  ...
  function makeRows(list, board) {
    return list.map(function (m) {
      var o = improveOuts(m.cards, board);
      return {
        cards: m.cards, key: m.cards.slice().sort().join(''), w: m.w,
        score: best5(m.cards.concat(board)),
        outs: o, outsEq: outsEquity(o),
        aw: 0, tw: 0, live: 0
      };
    });
  }
  ...
  function continueSet(an, f) {
    ...
    for (var i = 0; i < an.opp.length && acc < need - 1e-12; i++) {
      ...
      rows.push({ cards: r.cards, score: r.score, w: take, eq: r.eq });
      acc += take;
    }
  ...
  global.Equity = {
    ...
    analyze: analyze, continueSet: continueSet, vsSet: vsSet, blockers: blockers,
